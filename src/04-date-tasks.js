...
function timeSpanToString(startDate, endDate) {
  ...
  const convert = (num, countDigits) => num.toString().padStart(countDigits, '0');
  return `${convert(HH, 2)}:${convert(mm, 2)}:${convert(ss, 2)}.${convert(sss, 3)}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const hRotation = h > 11 ? (h - 12) * 30 + m / 2 : h * 30 + m / 2;
  const mRotation = m * 6;
  const angle = Math.abs(hRotation - mRotation) > 180
    ? (Math.abs(hRotation - mRotation - 180) / 180) * Math.PI
    : (Math.abs(hRotation - mRotation) / 180) * Math.PI;
  return angle;
}
